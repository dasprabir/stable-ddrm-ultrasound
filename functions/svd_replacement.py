...
class deconvolution_BCCB(H_functions):
    def __init__(self, kernel, dim, device):
        self.kernel = torch.tensor(kernel, dtype=torch.complex64)
        self.device = device
        self.channels = 3
        self.dim = dim
        ...
        sing_values = torch.tensor((np.fft.fft2(shifted_hest)), device=self.device, dtype=torch.complex64)
        sing_values = sing_values.unsqueeze(2).expand(-1, -1, self.channels).permute(2, 0, 1) # [C,H,W]

        # sing_values = sing_values.clone()
        # ZERO = 2e-4
        # sing_values[torch.abs(sing_values) < ZERO] = 0

        self.sing_values=sing_values.reshape(-1)

    def V(self, vec):
        ifft_result =  torch.real(torch.fft.ifft2(vec))
        return ifft_result.reshape(vec.shape[0],-1)

    def Vt(self, vec):
        fft_result =  torch.fft.fft2(vec)
    ...
    def H(self, vec):
        ...
        """
        temp = self.Vt(vec)
        S_Vt = (self.sing_values * temp[:, :self.sing_values.shape[0]]).reshape(vec.shape[0],self.channels,self.dim, self.dim)
        return self.U(S_Vt)
    ...
    """def Hinit(self, vec):
        ...
        S_Vt = (self.sing_values * temp[:, :self.sing_values.shape[0]]).reshape(vec.shape[0],self.channels,self.dim, self.dim)
        output = self.U(S_Vt)
        output = output.reshape(vec.shape[0],self.channels,self.dim, self.dim)
        return output"""
    ...
    def Ht(self, vec):
        ...
        """
        temp = self.Ut(vec)
        output = self.V(self.add_zeros(self.sing_values * temp[:, :self.sing_values.shape[0]]))
        return output
    ...
    def H_pinv(self, vec):
        ...
        """
        temp = self.Ut(vec)
        inv_singulars = torch.where(self.sing_values != 0, 1.0 / self.sing_values, torch.tensor(float('inf')))

        # Replace any infinities with a large value (e.g., 1000)
        inv_singulars = torch.where(torch.isinf(inv_singulars), torch.tensor(1000.0), inv_singulars)

        temp[:, :self.sing_values.shape[0]] = (temp[:, :self.sing_values.shape[0]] * inv_singulars)
