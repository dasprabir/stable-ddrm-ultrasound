...
def get_dataset(args, config):
    ...
    if config.data.dataset == "CELEBA":
    ...
    elif config.data.dataset == 'us_images2':
        from datasets.imagenet_subset import ImageDataset
        dataset = ImageDataset(os.path.join(args.exp, 'datasets', 'us_images2'),
                                    os.path.join(args.exp, 'deblur_us2.txt'),
                                    transform=transforms.Compose([transforms.ToTensor(), transforms.Resize([config.data.image_size, config.data.image_size])]),
                                    image_size=config.data.image_size,
                                    normalize=False)
        test_dataset = dataset
        ...
        from datasets.imagenet_subset import ImageDataset
        dataset = ImageDataset(root_dir=os.path.join(args.exp, 'datasets', 'MyRF'),
                                   meta_file=os.path.join(args.exp, 'MyRF.txt'),
                                   transform=transforms.Compose([transforms.ToTensor(), transforms.Resize([config.data.image_size, config.data.image_size])]),
                                   image_size=config.data.image_size,
                                   normalize=False)
        test_dataset = dataset
        ...
        from datasets.imagenet_subset import ImageDataset
        dataset = ImageDataset(os.path.join(args.exp, 'datasets', 'invivo'),
                                    os.path.join(args.exp, 'nantes.txt'),
                                    transform=transforms.Compose([transforms.ToTensor(), transforms.Resize([config.data.image_size, config.data.image_size])]),
                                    image_size=config.data.image_size,
                                    normalize=False)
        test_dataset = dataset
        ...
        from datasets.imagenet_subset import ImageDataset
        dataset = ImageDataset(os.path.join(args.exp, 'datasets', 'invivo'),
                                    os.path.join(args.exp, 'invivo.txt'),
                                    transform=transforms.Compose([transforms.ToTensor(), transforms.Resize([config.data.image_size, config.data.image_size])]),
                                    image_size=config.data.image_size,
                                    normalize=False)
        test_dataset = dataset
        ...
        from datasets.imagenet_subset import ImageDataset
        dataset = ImageDataset(os.path.join(args.exp, 'datasets', 'carotid'),
                                    os.path.join(args.exp, 'carotid.txt'),
                                    transform=transforms.Compose([transforms.ToTensor(), transforms.Resize([config.data.image_size, config.data.image_size])]),
                                    image_size=config.data.image_size,
                                    normalize=False)
        test_dataset = dataset
        ...
        from datasets.imagenet_subset import ImageDataset
        dataset = ImageDataset(os.path.join(args.exp, 'datasets', 'NewVivoData'),
                                    os.path.join(args.exp, 'NewVivoData.txt'),
                                    transform=transforms.Compose([transforms.ToTensor(), transforms.Resize([config.data.image_size, config.data.image_size])]),
                                    image_size=config.data.image_size,
                                    normalize=False)
        test_dataset = dataset
        ...
        from datasets.imagenet_subset import ImageDataset
        dataset = ImageDataset(os.path.join(args.exp, 'datasets', 'PICMUS','Simulation_Data'),
                                    os.path.join(args.exp, 'picmus.txt'),
                                    transform=transforms.Compose([transforms.ToTensor(), transforms.Resize([config.data.image_size, config.data.image_size])]),
                                    image_size=config.data.image_size,
                                    normalize=False)
        test_dataset = dataset
        ...
        from datasets.imagenet_subset import ImageDataset
        dataset = ImageDataset(os.path.join(args.exp, 'datasets', 'Besson_Data'),
                                    os.path.join(args.exp, 'BessonData.txt'),
                                    transform=transforms.Compose([transforms.ToTensor(), transforms.Resize([config.data.image_size, config.data.image_size])]),
                                    image_size=config.data.image_size,
                                    normalize=False)
        test_dataset = dataset
        ...
        from datasets.imagenet_subset import ImageDataset
        dataset = ImageDataset(os.path.join(args.exp, 'datasets', 'invivoGT'),
                                    os.path.join(args.exp, 'invivoGT.txt'),
                                    transform=transforms.Compose([transforms.ToTensor(), transforms.Resize([config.data.image_size, config.data.image_size])]),
                                    image_size=config.data.image_size,
                                    normalize=False)
        test_dataset = dataset
